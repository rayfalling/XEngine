...
- [ ] 4.1 提交 `fix: correct archetype swap-remove return semantics (slot row tracking)`，包含 `archetype.rs`（修改）与 `tests/go_mig_stress.rs`（新增，随本 fix 提交）。
- [ ] 4.2 提交 `docs(openspec): proposal for ecs swapmove slotrow fix`，包含 `openspec/changes/fix-ecs-swapmove-slotrow/` 目录。
- [ ] 4.3 **不提交** `crates/xengine-core/Cargo.toml`（新增 `[[bench]] name="go_access"`）与 `crates/xengine-core/benches/go_access.rs`（GO 层 spike），保留在工作区。
