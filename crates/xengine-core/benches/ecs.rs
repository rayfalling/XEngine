...
fn main() {
    ...
    println!("create({N}): {:?} ({:.2} ns/entity)", create_elapsed, create_elapsed.as_nanos() as f64 / N as f64);
    ...
    println!("iterate({N}): {:?} ({:.2} ns/entity)", iter_elapsed, iter_elapsed.as_nanos() as f64 / N as f64);
    ...
    for i in 0..N {
        world2.create2(Position(i as f32, 0.0), Velocity(1.0, 2.0)).unwrap();
    }
    let start = Instant::now();
    let mut sum = 0u64;
    world2.query2::<Position, Velocity>(|_e, _p, _v| sum += 1);
    let join_elapsed = start.elapsed();
    println!("query2({N}): {:?} ({:.2} ns/entity)", join_elapsed, join_elapsed.as_nanos() as f64 / N as f64);
